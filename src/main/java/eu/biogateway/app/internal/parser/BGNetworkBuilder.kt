...
class BGNetworkBuilder() {
    ...
    fun createAggregatedEdgeForRelationNode(netView: CyNetworkView, nodeView: View<CyNode>) {
        ...
        val query: BGRelationQuery = when (node.type.typeClass) {
            BGNodeType.BGNodeTypeClass.UNDIRECTED_STATEMENT -> BGFetchUndirectedRelationForBridgeQuery(node, relationIdentifier, fromUri, toUri)
            else -> {
                BGFetchRelationForBridgeQuery(node, relationIdentifier)
            }
        }
