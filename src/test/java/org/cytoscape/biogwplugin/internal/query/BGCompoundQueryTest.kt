...
internal class BGCompoundQueryTest {
    ...
    internal fun findBinaryPPITest() {
        val query = BGFindBinaryPPIsForMultipleNodesQuery(nodeUris) { query -> Thread(query).start() }
        Thread(query).start()
        ...
        for (relation in relations) {
            ...
            assertNotNull(BGServiceManager.cache.getRelationTypesForURI(relation.relationType.uri))
        }
    ...
    internal fun findCommonBinaryPPIsTest() {
        val query = BGFindBinaryPPIsForMultipleNodesQuery(nodeUris) { query -> query.run() }
